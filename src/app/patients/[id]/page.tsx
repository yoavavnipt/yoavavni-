    ...
    <AppLayout>
      <div style={{ padding:'20px 24px' }} className="fade-in">
          ...
          <div style={{ position:'fixed', inset:0, background:'rgba(0,0,0,0.5)', display:'flex', alignItems:'center', justifyContent:'center', zIndex:1000 }}>
            <div style={{ background:'#fff', borderRadius:'16px', padding:'28px', maxWidth:'380px', width:'90%', textAlign:'center' }}>
              ...
              <div style={{ fontSize:'13px', color:'#64748b', marginBottom:'24px', lineHeight:'1.6' }}>
                המטופל <strong>{patient.first_name} {patient.last_name}</strong> יועבר לארכיון.<br/>ניתן להחזיר בכל עת מדף הארכיון.
              </div>
              <div style={{ display:'flex', gap:'10px' }}>
                <button onClick={() => setShowArchiveConfirm(false)} style={{ flex:1, padding:'11px', border:'1px solid #e2e8f0', borderRadius:'10px', background:'#fff', fontSize:'13px', fontWeight:'600', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>ביטול</button>
                <button onClick={finishSeries} disabled={archiving} style={{ flex:1, padding:'11px', background:'#1a3a5c', color:'#fff', border:'none', borderRadius:'10px', fontSize:'13px', fontWeight:'800', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>{archiving ? '⏳...' : '✅ אשר סיום סדרה'}</button>
              </div>
            </div>
          </div>
        )}
          ...
          <div style={{ display:'flex', gap:'8px', flexWrap:'wrap' }}>
            {patient.phone && (() => {
              ...
              return (
                <>
                  {nextAppt && (
                    <button onClick={() => openWA(patient.phone, buildContractMsg(patient, nextAppt))} style={{ padding:'8px 12px', background:'#1e4a7a', color:'#fff', border:'none', borderRadius:'8px', fontSize:'11px', fontWeight:'700', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>📋 חוזה טיפולי</button>
                  )}
                  {nextAppt && (
                    <button onClick={() => openWA(patient.phone, buildReminderMsg(patient, nextAppt))} style={{ padding:'8px 12px', background:'#7c3aed', color:'#fff', border:'none', borderRadius:'8px', fontSize:'11px', fontWeight:'700', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>⏰ תזכורת לתור</button>
                  )}
                  ...
                  }} style={{ padding:'8px 12px', background:'#1a3a5c', color:'#fff', border:'none', borderRadius:'8px', fontSize:'11px', fontWeight:'700', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>🔗 הזמן לפורטל</button>
                </>
              )
            })()}
            ...
            <button onClick={() => setShowArchiveConfirm(true)} style={{ padding:'8px 14px', background:'#fee2e2', color:'#dc2626', border:'1px solid #fca5a5', borderRadius:'8px', fontSize:'12px', fontWeight:'700', cursor:'pointer', fontFamily:'Heebo, sans-serif' }}>📦 סיום סדרה</button>
          </div>
        </div>
        ...
        {records.filter(r => r.vas_score != null).length > 1 && (() => {
          ...
          const max = 10, w = 400, h = 80
          const pts = vasData.map((r, i) => ({ x: (i / Math.max(vasData.length - 1, 1)) * w, y: h - (r.vas_score / max) * h, score: r.vas_score, date: new Date(r.created_at).toLocaleDateString('he-IL', { day:'numeric', month:'numeric' }) }))
          const path = pts.map((p, i) => `${i === 0 ? 'M' : 'L'} ${p.x} ${p.y}`).join(' ')
          const area = `${path} L ${pts[pts.length-1].x} ${h} L 0 ${h} Z`
          const first = vasData[0]?.vas_score, last = vasData[vasData.length-1]?.vas_score
          ...
          return (
            <div style={{ background:'#fff', borderRadius:'12px', padding:'16px', marginBottom:'14px', boxShadow:'0 1px 4px rgba(0,0,0,0.06)' }}>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:'10px' }}>
                ...
                <span style={{ fontSize:'11px', fontWeight:'700', color: trendColor, background: last < first ? '#d1fae5' : last > first ? '#fee2e2' : '#fef3c7', padding:'3px 10px', borderRadius:'20px' }}>{trend}</span>
              </div>
              <svg viewBox={`0 0 ${w} ${h}`} style={{ width:'100%', height:'80px' }}>
                <defs><linearGradient id="vasGrad" x1="0" y1="0" x2="0" y2="1"><stop offset="0%" stopColor="#ef4444" stopOpacity="0.2" /><stop offset="100%" stopColor="#ef4444" stopOpacity="0.02" /></linearGradient></defs>
                <path d={area} fill="url(#vasGrad)" />
                <path d={path} fill="none" stroke="#ef4444" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round" />
                {pts.map((p, i) => (<g key={i}><circle cx={p.x} cy={p.y} r="4" fill="#ef4444" /><text x={p.x} y={p.y - 8} textAnchor="middle" fontSize="10" fill="#374151" fontWeight="bold">{p.score}</text></g>))}
              </svg>
        ...
        <div style={{ display:'flex', borderBottom:'2px solid #f1f5f9', marginBottom:'16px', overflowX:'auto' }}>
          {tabs.map(t => (
            <button key={t.key} onClick={() => setTab(t.key as any)} style={{ padding:'10px 14px', border:'none', background:'none', cursor:'pointer', whiteSpace:'nowrap', fontSize:'13px', fontWeight:tab===t.key?'700':'400', color:tab===t.key?'#1a3a5c':'#94a3b8', borderBottom:tab===t.key?'2px solid #1a3a5c':'2px solid transparent', marginBottom:'-2px', fontFamily:'Heebo, sans-serif' }}>{t.label}</button>
          ))}
        </div>
        ...
        {tab === 'overview' && (
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'14px' }}>
            ...
            <div style={card}>
              ...
              {editing ? (
                <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:'10px' }}>
                  <div style={{gridColumn:'1/-1'}}><label style={lbl}>סוג מימון</label><select style={inp} value={form.funding_type||'self'} onChange={e=>set('funding_type',e.target.value)}><option value="self">עצמי</option><option value="hmo">קופת חולים</option><option value="insurance">ביטוח פרטי</option><option value="group">קבוצה</option></select></div>
                  {form.funding_type==='hmo' && <div style={{gridColumn:'1/-1'}}><label style={lbl}>קופת חולים</label><select style={inp} value={form.hmo||''} onChange={e=>set('hmo',e.target.value)}><option value="">בחר...</option>{HMO_OPTIONS.map(h=><option key={h}>{h}</option>)}</select></div>}
                  {form.funding_type==='insurance' && <div style={{gridColumn:'1/-1'}}><label style={lbl}>חברת ביטוח</label><input style={inp} value={form.insurance||''} onChange={e=>set('insurance',e.target.value)} placeholder="מגדל, כלל, מנורה..."/></div>}
                  {form.funding_type==='group' && <div style={{gridColumn:'1/-1'}}><label style={lbl}>שם הקבוצה</label><input style={inp} value={form.funding_group_name||''} onChange={e=>set('funding_group_name',e.target.value)} placeholder="מכבי, צבא, עבודה..."/></div>}
                ...
                <div style={{display:'flex',flexDirection:'column',gap:'8px'}}>
                  {[['גורם מממן',fundingLabel],['אבחנה',patient.diagnosis],['רקע רפואי',patient.medical_history],['תרופות',patient.medications],['אלרגיות',patient.allergies]].filter(([,v])=>v).map(([l,v])=>(
                    <div key={l as string} style={{display:'flex',gap:'8px',fontSize:'13px'}}><span style={{color:'#94a3b8',minWidth:'80px',flexShrink:0}}>{l}</span><span style={{fontWeight:'500',color:'#1e293b'}}>{v}</span></div>
                  ))}
                </div>
              )}
          ...
          <div>
            ...
            <div style={card}>
              ...
              <div style={{display:'flex',flexDirection:'column',gap:'10px'}}>
                {[{key:'exercise_explained',label:'הוסברה חשיבות תרגול עצמי'},{key:'no_ci',label:'אין CI'},{key:'no_red_flags',label:'שולל דגלים אדומים'},{key:'no_violence',label:'שולל אלימות'},{key:'patient_consent',label:'המטופל נתן הסכמתו להצבת המטרות'}].map(item => (
                  <label key={item.key} style={{display:'flex',alignItems:'center',gap:'10px',cursor:'pointer',fontSize:'13px',fontWeight:'500'}}>
                    <input type="checkbox" checked={!!intake[item.key]} onChange={e=>setI(item.key,e.target.checked)} style={{width:'18px',height:'18px',cursor:'pointer'}}/>{item.label}
                  </label>
                ))}
              </div>
            </div>
            <div style={{display:'flex',justifyContent:'flex-end',paddingBottom:'20px'}}>
              <button onClick={saveIntake} disabled={intakeSaving} style={{padding:'10px 28px',background:intakeSaving?'#94a3b8':'#1a3a5c',color:'#fff',border:'none',borderRadius:'8px',fontSize:'14px',fontWeight:'700',cursor:'pointer',fontFamily:'Heebo, sans-serif'}}>{intakeSaving?'⏳ שומר...':'💾 שמור ראיון קבלה'}</button>
            </div>
          </div>
        )}
                ...
                <div key={g.id} style={{ background:'#fff', borderRadius:'12px', padding:'16px', marginBottom:'10px', boxShadow:'0 1px 4px rgba(0,0,0,0.06)', borderRight: g.status === 'הושג' ? '4px solid #10b981' : '4px solid #3eb8e5' }}>
                  ...
                  {updates.length > 0 && (
                    <div style={{ marginBottom:'10px', padding:'10px', background:'#f8fafc', borderRadius:'8px' }}>
                      ...
                      {updates.map((u,i) => (<div key={u.id} style={{ display:'flex', gap:'8px', fontSize:'12px', marginBottom: i < updates.length-1 ? '6px' : '0' }}><span style={{ color:'#94a3b8', flexShrink:0 }}>{new Date(u.date).toLocaleDateString('he-IL')}</span><span style={{ color:'#374151' }}>{u.note}</span></div>))}
                    </div>
                  )}
                  {g.status !== 'הושג' && (
