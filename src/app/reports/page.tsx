...
export default function ReportsPage() {
  ...
  async function load() {
    ...
    else if (period === 'quarter') startDate = new Date(now.getFullYear(), Math.floor(now.getMonth()/3)*3, 1).toISOString()
    ...
    paid.forEach(b => { const m=b.payment_method||'אחר'; paymentMethods[m]=(paymentMethods[m]||0)+(b.amount||0) })

    setStats({ totalPatients, activePatients, totalIncome, avgPerSession, appointments, paidCount:paid.length, paymentMethods })
    setLoading(false)
  }

  const periodLabels = { month:'חודש זה', quarter:'רבעון זה', year:'שנה זו' }
          ...
          <div style={{ display:'flex', background:'#fff', borderRadius:'8px', border:'1px solid #e2e8f0', overflow:'hidden' }}>
            {(['month','quarter','year'] as const).map(p => (
              <button key={p} onClick={()=>setPeriod(p)} style={{
                ...
                {{'month':'חודש','quarter':'רבעון','year':'שנה'}[p]}
              </button>
            ))}
          </div>
        </div>
        ...
        {loading ? (
          ...
          <>
            <div style={{ display:'grid', gridTemplateColumns:'repeat(2,1fr)', gap:'12px', marginBottom:'20px' }}>
              {[
                { label:'הכנסה', value:`₪${(stats.totalIncome||0).toLocaleString()}`, icon:'💰', color:'#0b8a5e', sub:periodLabels[period] },
                { label:'ממוצע לטיפול', value:`₪${stats.avgPerSession||0}`, icon:'📊', color:'#3eb8e5', sub:`${stats.paidCount||0} טיפולים` },
                { label:'תורים', value:stats.appointments||0, icon:'📅', color:'#7c3aed', sub:periodLabels[period] },
                { label:'מטופלים פעילים', value:stats.activePatients||0, icon:'👥', color:'#1e4a7a', sub:`מתוך ${stats.totalPatients||0} סה"כ` },
              ].map(k => (
                  ...
                  return (
                    <div key={method} style={{ marginBottom:'12px' }}>
                      ...
                      <div style={{ background:'#f1f5f9', borderRadius:'10px', height:'6px', overflow:'hidden' }}>
                        <div style={{ width:`${pct}%`, height:'100%', background:'#1a3a5c', borderRadius:'10px' }}/>
                      </div>
                    </div>
                  )
                })}
