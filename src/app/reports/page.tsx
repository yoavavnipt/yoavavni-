...
export default function ReportsPage() {
  ...
  async function load() {
    ...
    paid.forEach(b => {
      ...
      svcRevenueMap[svc].revenue += (b.amount || 0)
      svcRevenueMap[svc].count++
    })
    ...
    ;(allBillingEver || []).filter(b => b.status === 'paid').forEach(b => {
      ...
      const name = `${(b.patient as any)?.first_name} ${(b.patient as any)?.last_name}`
      if (!ltvMap[pid]) ltvMap[pid] = { name, total: 0, count: 0, first: b.created_at }
      ltvMap[pid].total += (b.amount || 0)
      ltvMap[pid].count++
    ...
    // עונתיות — הכנסה לפי חודש בשנה (כל השנים)
    const seasonMap: Record<number, number> = {}
    ;(allBillingEver || []).filter(b => b.status === 'paid').forEach(b => {
      const month = parseInt(b.created_at?.slice(5, 7) || '1')
      seasonMap[month] = (seasonMap[month] || 0) + (b.amount || 0)
    })
    setSeasonality(Array.from({ length: 12 }, (_, i) => ({ label: MONTH_NAMES[i], value: Math.round((seasonMap[i + 1] || 0) / 1000) })))

    // סיכון עסקי — תלות בלקוח
...
- שירותים מובילים: ${reportData.serviceRevenue?.map((s: any) => `${s[0]} (₪${s[1].revenue})`).join(', ')}

ספק:
1. **3 תובנות עיקריות** — מה בולט בנתונים
2. **2 הזדמנויות עסקיות** — מה אפשר לשפר
3. **1 סיכון** שכדאי לשים לב אליו
4. **המלצה אחת קונקרטית** לחודש הקרוב

תהיה ספציפי, מעשי, ותתייחס לנתונים הממשיים.`
        ...
        {loading ? (
          ...
          <>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px', marginBottom: '16px' }}>
              <div style={{ background: '#fff', borderRadius: '12px', padding: '18px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '10px' }}>
                  ...
                  {riskData.topPct > 20 && <div style={{ fontSize: '11px', color: '#dc2626', padding: '8px', background: '#fee2e2', borderRadius: '6px' }}>⚠️ תלות גבוהה — מטופל אחד מייצג יותר מ-20% מההכנסות</div>}
                </div>
              </div>
            ...
            <div style={{ background: '#fff', borderRadius: '12px', padding: '18px', marginBottom: '14px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
              <div style={{ fontWeight: '700', fontSize: '13px', color: '#1a3a5c', marginBottom: '14px' }}>🌊 עונתיות — הכנסות לפי חודש בשנה (₪ אלפים)</div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: '14px' }}>
              <div style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                {noShows.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: '#94a3b8', fontSize: '12px' }}>אין no-shows בתקופה זו</div> : noShows.map((n, i) => (
              ...
              <div style={{ background: '#fff', borderRadius: '12px', overflow: 'hidden', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
                ...
                {cancellations.length === 0 ? <div style={{ padding: '20px', textAlign: 'center', color: '#94a3b8', fontSize: '12px' }}>אין ביטולים בתקופה זו</div> : cancellations.map(([name, count], i) => (
                  <div key={i} style={{ display: 'flex', justifyContent: 'space-between', padding: '10px 18px', borderBottom: '1px solid #f8fafc', fontSize: '13px' }}>
